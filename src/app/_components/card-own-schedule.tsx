...
export const CardOwnSchedule = ({ schedule }: CardOwnScheduleProps) => {
  return (
    <div
      key={schedule.id}
      ...
      <div className="flex flex-wrap justify-between gap-4 sm:flex-row sm:items-center">
        <div className="space-y-2">
          ...
          <p className="bg-accent mr-2 inline-flex items-center justify-center rounded-md p-2 shadow-md">
            {SetInstrument(schedule.participant.instrument as Instrument).icon}
            <span className="ml-2">
              {
                SetInstrument(schedule.participant.instrument as Instrument)
                  .label
              }
            </span>
          </p>
        ...
        <div className="space-y-2">
          ...
          <p className="p-2">{schedule.participant.justification ?? "-"}</p>
        </div>
        ...
        <div className="space-y-2">
          ...
          <Badge
            variant="secondary"
            className={`p-2 ${schedule.participant.confirmed === false && "bg-destructive/50"} ${schedule.participant.confirmed === null && "bg-chart-5/50"} ${schedule.participant.confirmed && "bg-green-500/50"}`}
          >
            {schedule.participant.confirmed && "Confirmado"}
            {schedule.participant.confirmed === false && "Rejeitado"}
            {schedule.participant.confirmed === null && "Pendente"}
          </Badge>
        </div>
      </div>
    </div>
